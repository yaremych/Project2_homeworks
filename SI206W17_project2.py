...
## Anyone you worked with on this project: None
...
# later: write the code for grabbing info from UMSI/twitter into a function that checks for the request, caches (like HW 5)
...
# pattern 1: 'http://' then some letters '[a-z]*', then a period '\.', then at least 2 more characters -- some have just letters, some have additinoal periods'[a-z.]+'
# pattern 2: 'https:// then same stuff'

def find_urls(string):
	http_results = re.findall('http://[a-z]*\.[a-z.]+' , string)
	https_results = re.findall('https://[a-z]*\.[a-z.]+' , string)

	results = []
	for x in http_results:
		results.append(x)
	for x in https_results:
		results.append(x)
...
for tweet in five_tweets:
	url_results = find_urls(tweet)

	for item in url_results:
		tweet_urls_found.append(item)
